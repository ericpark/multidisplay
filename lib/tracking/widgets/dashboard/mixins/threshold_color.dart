  ...
  Color getThresholdColor({
    ...
    try {
      if (thresholds.isEmpty) {
        throw ArgumentError('No thresholds found');
      }
      if (thresholds['good'] == null) {
        throw ArgumentError('Good threshold not found');
      }
      if (thresholds['warn'] == null) {
        throw ArgumentError('Warn threshold not found');
      }
      if (thresholds['poor'] == null) {
        throw ArgumentError('Poor threshold not found');
      }
      if (_gapsInThreshold(thresholds: thresholds)) {
        throw ArgumentError('No Gaps allowed');
      }
      if (_invalidThresholdOrder(thresholds: thresholds)) {
    ...
    final higherIsPositive = _higherIsPositive(thresholds: thresholds);

    /// Determine if the compareMetric is within the threshold
    ///
    /// Given a threshold ("good", "warn", "poor") return true if the
    /// compareMetric is within the range. This works for both directions.
